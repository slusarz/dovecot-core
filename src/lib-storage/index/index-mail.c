...
static int index_mail_write_body_snippet(struct index_mail *mail)
{
	...
	string_t *str;
	int ret;
	const char *driver_name = mail->mail.mail.box->storage->set->mail_snippet_driver;
	const struct message_snippet_driver *driver;

	i_assert(mail->data.parsed_bodystructure);

	driver = message_snippet_driver_find(driver_name);
	if (driver == NULL)
		driver = message_snippet_driver_find("internal");
	if (driver == NULL) {
		/* should not happen */
		return -1;
	}
	...
	str = str_new(mail->mail.data_pool, 128);
	ret = driver->generate(&mail->mail.mail, input, BODY_SNIPPET_MAX_CHARS, str);
	if (ret == 0)
